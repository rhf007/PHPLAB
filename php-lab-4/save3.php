...
if(empty($errors)){
    ...
    //save to database:
    $stmt = $mysqli->prepare("INSERT INTO userdata (name, email, room, ext, img) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $username, $email, $room, $ext, $imgfilename);
    $stmt->execute();
    if ($stmt->error) {
    ...
    header("location:register3.php?errors=$errorsStr");
}
